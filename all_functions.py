...
def bleau_score(answer, context):
  answer = answer.split(' ')
  if isinstance(context, list): 
      context = context.split(' ')
      context = list(context)

  bleu_score = sentence_bleu(context, answer)  
  return bleu_score


def return_bert_score(answer, context, lang="en"):
  
  # hypotheses = ["The cat sat on the mat."]
  # references = ["The cat is sitting on the mat."]
  if not isinstance(answer, list):   answer  = list(answer)
  if not isinstance(context, list) : context = list(context)
      
  
  P, R, f1 = score(answer, context, lang=lang, verbose=True)
